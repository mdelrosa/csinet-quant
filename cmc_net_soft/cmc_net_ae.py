...
from cmc_net import Encoder, Decoder, DeepCMCQuant
...
from models.latent_quantizers import SoftQuantize
...
class DeepCMCArithmeticEncoding(DeepCMCQuant):
    """ DeepCMC for csi estimation with arithmetic encoding """
    def __init__(self, encoder, decoder, quant, rate, **kwargs):
        DeepCMCQuant.__init__(self, encoder, decoder, quant, rate, **kwargs)
    ...
    def forward(self, H_in):
        """ forward call for DeepCMC-SoftQuant with AE """
        h_enc = self.encoder(H_in)
        b = h_enc.shape[0]
        h_enc = h_enc.view(self.batch_size,-1) # flatten latent layer
        q = self.quant(h_enc)
...
if __name__ == "__main__":
    ...
    from trainer import count_centers, arithmetic_encoding
    ...
    learning_rate = 1e-5
    batch_size = 200 # hard code batch size; 200 causing memory issues on g6-8
    ...
    deepcmc = DeepCMCQuant(encoder, decoder, quant, opt.rate, batch_size=batch_size, device=device).to(device)
    ...
    print(f"Loading best model weights from {pickle_dir}/{network_name}-soft-best-model.pt")

    deepcmc.quant.init_centers(torch.zeros(deepcmc.quant.L, deepcmc.quant.m).to(device))
    deepcmc.load_state_dict(torch.load(f"{pickle_dir}/{network_name}-soft-best-model.pt", map_location=device), strict=False)

    deepcmc_ae = DeepCMCArithmeticEncoding(deepcmc.encoder, deepcmc.decoder, deepcmc.quant, opt.rate, device=device).to(device) # remake network with non-trainable sigma
    ...
    count_centers(deepcmc_ae,
                    valid_ldr,
                    batch_num,
                    timers=timers,
                    json_config=json_config,
                    debug_flag=opt.debug_flag,
                    str_mod=f"DeepCMC-Soft (after training centers)",
    ...
    deepcmc_ae.quant.return_denorm = False # for calculating histogram
    deepcmc_ae.return_latent = False
    deepcmc_ae.count_centers = False
