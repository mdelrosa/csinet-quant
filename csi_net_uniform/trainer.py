...
def fit(model, train_ldr, valid_ldr, batch_num, schedule=None, criterion=nn.MSELoss(), epochs=10, timers=None, json_config=None, torch_type=torch.float, debug_flag=True, pickle_dir=".", input_type="split", patience=1000, network_name=None, quant_bool=False, anneal_bool=False, l2_weight=1e-12, data_all=None, timeslot=0,):
    # pull out timers
    ...
    with predict_timer:
        ...
        with torch.no_grad():
            ...
            #     inputs = autograd.Variable(data_batch)
                ...
                #     entropy = model.crossentropy_loss(model_in)
                #     test_entropy += entropy
                # print(f"model.z_min_temp: {model.z_min_temp} - model.z_max_temp: {model.z_max_temp}")
                z_min = np.min([z_min, model.z_min_temp])
                z_max = np.max([z_max, model.z_max_temp])
...
def update_histogram(model, valid_ldr, batch_num, timeslot=0, timers=None, json_config=None, debug_flag=True, str_mod="", entropy_coding=False):
    """
